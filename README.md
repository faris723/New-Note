...
5. **Untuk merilis versi resmi** yang bisa dideteksi fitur "Cek Update" di dalam aplikasi: isi `release_tag` dengan nomor versi diawali huruf `v`, misalnya `v1.0.4`. Workflow otomatis akan:
   - Build APK
   - Membuat tag Git `v1.0.4`
...
| Lokasi | Contoh nilai untuk rilis v1.0.4 |
|---|---|
| `src/version.js` → `APP_VERSION` | `'1.0.4'` |
| `android/app/build.gradle` → `versionName` | `"1.0.4"` |
| `android/app/build.gradle` → `versionCode` | naikkan, untuk 1.0.4 menjadi `5` |
| `release_tag` saat menjalankan workflow | `v1.0.4` |
